...
// 	},
    ...
    if(this.options.zoom_enabled){
		...
		// this.options.chromwidth = 2 * this.plotCircleSize;	
  ...
  _removeChroms:function(){
	...
	_drawChroms: function(){
		...
		var chrompath = function(chromosome){
			...
			if(chromosome.centromere_start != undefined){
				returnstr= "M0," + widget.yconversion(chromosome.offset) +" C0,"+ (widget.yconversion(chromosome.offset)-widget.options.capheight).toString() + " " + widget.options.chromwidth + ","+ (widget.yconversion(chromosome.offset)-widget.options.capheight).toString() + " " + widget.options.chromwidth + "," + widget.yconversion(chromosome.offset) +
			" V" + widget.yconversion(chromosome.centromere_start+chromosome.offset) +
			" L" + (widget.options.chromwidth-widget.options.chromwidth/widget.options.centromereIndentation).toString() + "," + widget.yconversion((chromosome.centromere_end-chromosome.centromere_start)/2+chromosome.centromere_start+chromosome.offset) +
			" L" + widget.options.chromwidth + "," + widget.yconversion(chromosome.centromere_end+chromosome.offset) +
			" V" + widget.yconversion(chromosome.size+chromosome.offset) + 
			" C" + widget.options.chromwidth + "," + (widget.yconversion(chromosome.size+chromosome.offset)+widget.options.capheight).toString() + " 0," + (widget.yconversion(chromosome.size+chromosome.offset)+widget.options.capheight).toString()  +
			" 0," + widget.yconversion(chromosome.size+chromosome.offset) + 
			" V" + widget.yconversion(chromosome.centromere_end+chromosome.offset) +
			" L" + widget.options.chromwidth/widget.options.centromereIndentation + "," +   widget.yconversion((chromosome.centromere_end-chromosome.centromere_start)/2+chromosome.centromere_start+chromosome.offset) +
			" L0," + widget.yconversion(chromosome.centromere_start+chromosome.offset) + 
			" V" + widget.yconversion(chromosome.offset);				
			}
			else{
				returnstr= "M0," + widget.yconversion(chromosome.offset) +" C0,"+ (widget.yconversion(chromosome.offset)-widget.options.capheight).toString() + 
					" " + widget.options.chromwidth + ","+ (widget.yconversion(chromosome.offset)-widget.options.capheight).toString() + 
					" " + widget.options.chromwidth + "," + widget.yconversion(chromosome.offset) +
					" V" + widget.yconversion(chromosome.size+chromosome.offset) + 
					" C" + widget.options.chromwidth + "," + (widget.yconversion(chromosome.size+chromosome.offset)+widget.options.capheight).toString() + " 0," + (widget.yconversion(chromosome.size+chromosome.offset)+widget.options.capheight).toString() +
						" 0," + widget.yconversion(chromosome.size+chromosome.offset) + 
					" V" + widget.yconversion(chromosome.offset);
		...
		var leftCent = function(chromosome){
			var points;
			points = "0," + widget.yconversion(chromosome.centromere_start + chromosome.offset) 
				+ " " +  widget.options.chromwidth/widget.options.centromereIndentation + "," + 
				widget.yconversion((chromosome.centromere_end-chromosome.centromere_start)/2+chromosome.centromere_start+chromosome.offset) +
				" 0," + widget.yconversion(chromosome.centromere_end + chromosome.offset);
			return points;
		}
		
		var rightCent = function(chromosome){
			var points;
			points = widget.options.chromwidth + "," + widget.yconversion(chromosome.centromere_start + chromosome.offset) +
			" " + (widget.options.chromwidth - widget.options.chromwidth/widget.options.centromereIndentation).toString() +
			"," + widget.yconversion((chromosome.centromere_end-chromosome.centromere_start)/2+chromosome.centromere_start+chromosome.offset) +
			" " + widget.options.chromwidth + "," +  widget.yconversion(chromosome.centromere_end + chromosome.offset);
			return points;
		}
		...
		this.chromd3.append("text")
			...
			.attr("x", widget.options.chromwidth/2)
			.attr("y", function(d){return widget.yconversion(d.size+d.offset) + widget.options.capheight*2.5;})
	...
	_drawPhenos: function(){
		...
		for(j=0; j<widget.options.chroms[i].positions.length;j++){
			...
			widget.options.chroms[i].positions[j].x = widget.options.chromwidth+widget.options.linelength;
			widget.options.chroms[i].positions[j].y = this.yconversion(widget.options.chroms[i].positions[j].pos + widget.options.chroms[i].offset);
			widget.options.chroms[i].positions[j].orig_y = widget.options.chroms[i].positions[j].y;
			nodes.push({x: widget.options.chromwidth, y:widget.options.chroms[i].positions[j].y, fixed: true})
...
// 											.theta(0)
											...
											.charge(-85)
// 											.chargeDistance(circleradius*2)
											.nodes(nodes)
											.links(links)
											.on("tick", function(){
												for(var ni=0; ni<nodes.length; ni++){
													if(nodes[ni].x != widget.options.chromwidth){
														nodes[ni].x = widget.options.chromwidth+widget.options.linelength;
													}
		...
		for(z=0; z<widget.options.chroms[i].positions.length; z++){
			widget.options.chroms[i].positions[z].y = nodepositions[z];
		}
		...
		circles.attr("class", "phenocircle")
			...
			.on("click", function(d){
				var ph={};
				ph[d.pheno]=1;
console.log(d.pheno + " ph keys=" + Object.keys(ph));
	...
	_drawTransverseLines: function(){
		var widget = this;
		if(!this.options.chromsOnly){
			var lineLength = widget.options.chromwidth +  widget.options.linelength - widget.options.circleradius;
			var linkLines = this.chromd3.selectAll("line")
				...
				.attr("x1", widget.options.chromwidth)
		...
		var transversePath = function(position, widget){
			...
			var dpath = "M0," + position.orig_y + " H" + widget.options.chromwidth;
			// add lower portion when needed
			if(position.endPos != undefined){
				dpath += " V" + widget.yconversion(chromOffset + position.endPos) +
					" H0 V" + position.orig_y + " Z";
			}
			return dpath;
		}

		// use svg path and can then fill in (I think) 
...
// 		this.options.ymax = this._calcYmax();
	...
	_placeChroms: function(){
		...
		var nChromsIncluded = this.includedChroms.length;
		var includedChromMap = {};
		if(nChromsIncluded ==  0){
			nChromsIncluded = this.options.chroms.length;
			for(var i=0; i<this.options.chroms.length; i++){
				this.options.chroms[i].visible = true;
				this.includedChroms.push(this.options.chroms[i].name);
				includedChromMap[this.includedChroms[i].toString()]=true;
			}
		}
		else{
			for(var i=0; i<this.includedChroms.length; i++){
				includedChromMap[this.includedChroms[i].toString()]=true;
			}
			for(var i=0; i<this.options.chroms.length; i++){
		...
		var avail_y = this.options.chromboxwidth - this.options.margin.left - this.options.margin.right;
		
		// need to calculate circle size and then chromwidth
		// size is 32 for 12 chroms across and then increases from there 
		this.plotCircleSize = this.options.circlesize + (this.options.maxChromsRow/2 - Math.floor(this.options.chromsperrow/2))*2;
		this.options.circleradius = this.plotCircleSize/2;
		
		this.options.chromwidth = (2 + (12 - this.options.chromsperrow) * 0.1) * this.plotCircleSize;
		
		this.options.circlesPerChrom = Math.floor((avail_y-this.options.chromwidth-this.options.linelength)/this.plotCircleSize) + 1;
		...
		this.yconversion = d3.scale.linear()
			...
			.domain([0, this.maxbp]);
		for(var i=0; i<this.options.chroms.length; i++){
			this.options.chroms[i].offset = (this.maxbp - this.options.chroms[i].size)/2;
		}
		
		var widget=this;
		this.chromd3 = this.canvas.selectAll("g")
			.data(function(d){
				if(widget.options.chroms.length == widget.includedChroms.length){
		...
		for(var i=0; i<this.options.phenos.length; i++){
			...
			if(chr.posMap[pheno.position.toString()]==undefined){
					chr.positions.push({pos: pheno.position, endPos: pheno.endPosition, count: 0, 
						id:pheno.id, chrom: pheno.chrom, posColor: pheno.posColor});
	...
	highlightPhenos: function(phenopos, reset){
		...
		if(Object.keys(phenopos).length==0)
			reset = true;
// 		if(widget.highlighted_pheno == phenopos.pheno){
// console.log("reset=" + reset);
		if(reset){
// 			d3.selectAll(".chromLinker")
// 				.attr("stroke", "gray")
...
console.log('not resetting ' + Object.keys(phenopos));
// 			d3.selectAll(".chromLinker")
// 				.attr("stroke", function(d){
// // 					if(d.pheno != phenopos.pheno)
// 					if(phenopos[d.pheno])
// 						return 'gray';
// 					else
// // 						return 'lightgray';
...
// 						return widget.phenoColors(phenopos.pheno);
						return 'lightgray';
					}
				})
			 	.each("end", function(){
...
// 		this.highlightPhenos({pheno:this.highlighted_pheno}, true);
	...
	drawCytoBands: function(){
			...
			widget.chromd3.selectAll(".cytoband")
				...
				.attr("points", function(d){
					 ...
					 return "1, " + widget.yconversion(d.startBand+chr.offset)  + " "
					+ (widget.options.chromwidth-1).toString() + "," + widget.yconversion(d.startBand+chr.offset) + " "
					+ (widget.options.chromwidth-1).toString() + "," + widget.yconversion(d.finishBand+chr.offset) + " "
					+ "1," + widget.yconversion(d.finishBand+chr.offset);})
				.attr("stroke", "none")
				.attr("stroke-width", 1)
	...
	showCytoBands: function(){
		this.chromd3.selectAll(".chromOutline")
			.attr("opacity", 0);
		this.chromd3.selectAll(".cytoband")
			.attr("opacity", 0.7);
		this.chromd3.selectAll(".chromOutline")
	...
	console.log("triggering phenoSelection " + phenoname);
		this._trigger('select',{type: 'select'}, {phenoName: phenoname});
		
// 		this._trigger('selected', {type: 'selected'}, {keyNames: this.selectedKeys});
	},
	...
	_addPhenoKey:function(xoffset, yoffset, zoom, phenoRowInfo){
		...
		var widget = this;
		var currPos = 0;
		var yInterval = this.options.ymax / 40;
		var xInterval = this.options.xmax / (phenoRowInfo.phenosPerRow+1);
